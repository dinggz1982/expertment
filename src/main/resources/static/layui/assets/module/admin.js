...
layui.define(["layer"], function (f) {
    ...
    n.popupRight = function (i) {
        ...
        i.move = false;
        i.fixed = true;
        (i.area === undefined) && (i.area = "336px");
        (i.title === undefined) && (i.title = false);
        (i.closeBtn === undefined) && (i.closeBtn = false);
        (i.shadeClose === undefined) && (i.shadeClose = true);
        (i.skin === undefined) && (i.skin = "layui-anim layui-anim-rl layui-layer-adminRight");
        return n.open(i)
    };
    n.open = function (r) {
        (r.area === undefined) && (r.area = (r.type === 2 ? ["360px", "300px"] : "360px"));
        (r.offset === undefined) && (r.offset = "70px");
        (r.shade === undefined) && (r.shade = 0.1);
        (r.fixed === undefined) && (r.fixed = false);
        (r.resize === undefined) && (r.resize = false);
        (r.skin === undefined) && (r.skin = "layui-layer-admin");
        var p = r.end;
        r.end = function () {
            k.closeAll("tips");
            p && p()
        ...
        if (r.url) {
            ...
            r.success = function (s, t) {
                n.reloadLayer(s, r.url, q)
            }
        }
        var i = k.open(r);
        (r.data) && (n.layerData["d" + i] = r.data);
        return i
    };
    n.getLayerData = function (i, p) {
        if (i === undefined) {
            i = parent.layer.getFrameIndex(window.name);
            (i !== undefined) && (i = parseInt(i));
            return parent.layui.admin.getLayerData(i, p)
        } else {
            if (isNaN(i)) {
                var q = h(i).parents(".layui-layer").attr("id");
                q && (i = q.substring(11))
            }
        }
        var r = n.layerData["d" + i];
        if (p) {
            return r ? r[p] : undefined
        }
        return r
    };
    n.putLayerData = function (p, r, i) {
        if (i === undefined) {
            i = parent.layer.getFrameIndex(window.name);
            (i !== undefined) && (i = parseInt(i));
            return parent.layui.admin.putLayerData(p, r, i)
        } else {
            if (isNaN(i)) {
                i = h(i).parents(".layui-layer").attr("id").substring(11)
            }
        }
    ...
    n.reloadLayer = function (p, i, r) {
        ...
        (i === undefined) && (i = q.data("url"));
        q.data("url", i);
        n.showLoading(q);
        n.ajax({
            url: i, dataType: "html", success: function (s) {
                n.removeLoading(q, false);
                q.children(".layui-layer-content").html(s);
                r && r(q[0], p)
            }
        })
    };
    n.req = function (i, p, q, r) {
        ...
        if (d.reqPutToPost) {
            if ("put" === r.toLowerCase()) {
                r = "POST";
                p._method = "PUT"
            } else {
                if ("delete" === r.toLowerCase()) {
                    r = "GET";
                    p._method = "DELETE"
                }
            }
        }
        n.ajax({url: (d.baseServer || "") + i, data: p, type: r || "GET", dataType: "json", success: q})
    };
    n.ajax = function (t) {
        ...
        t.error = function (i) {
            t.success({code: i.status, msg: i.statusText}, undefined, i)
        };
    ...
    n.showLoading = function (s, r, p, q) {
        ...
        (p !== undefined) && (t.css("background-color", "rgba(255,255,255," + p + ")"));
        t.show()
    };
    ...
    n.putTempData = function (p, q) {
        var i = d.tableName + "_tempData";
        if (q === undefined || q === null) {
            layui.sessionData(i, {key: p, remove: true})
        } else {
            layui.sessionData(i, {key: p, value: q})
        }
    };
    n.getTempData = function (p) {
        var i = d.tableName + "_tempData";
        var q = layui.sessionData(i);
        return q ? q[p] : undefined
    };
    n.rollPage = function (r) {
        if (window !== top && !n.isTop() && top.layui && top.layui.admin) {
            top.layui.admin.rollPage(r);
            return
        }
    ...
    n.refresh = function (i) {
        ...
        if (q && q[0]) {
            try {
                q[0].contentWindow.location.reload(true)
            } catch (p) {
    ...
    n.closeThisTabs = function (i) {
        if (window !== top && !n.isTop() && top.layui && top.layui.admin) {
            top.layui.admin.closeThisTabs(i);
            return
        }
        n.closeTabOperNav();
        var p = h(l + ">.layui-tab-title");
    ...
    n.closeOtherTabs = function (i) {
        if (window !== top && !n.isTop() && top.layui && top.layui.admin) {
            top.layui.admin.closeOtherTabs(i);
            return
        }
        if (!i) {
    ...
    n.closeDialog = function (i) {
        var p = h(i).parents(".layui-layer").attr("id").substring(11);
        k.close(p)
    };
    ...
    n.hideTableScrollBar = function (q) {
        ...
        if (!q) {
            ...
            (p.length <= 0) && (p = h(b + ">div>.admin-iframe"));
            (p.length > 0) && (q = p[0].contentWindow)
        }
        try {
            (window.hsbTimer) && (clearTimeout(hsbTimer));
    ...
    n.events = {
        flexible: function () {
            ...
            n.strToWin(h(this).data("window")).layui.admin.popupRight({
                id: "layer-theme",
                url: i ? i : "page/tpl/tpl-theme.html"
            })
        }, note: function () {
            var i = h(this).data("url");
            n.strToWin(h(this).data("window")).layui.admin.popupRight({
                ...
                type: 2,
                closeBtn: false,
                content: i ? i : "page/tpl/tpl-note.html"
            })
        }, message: function () {
            var i = h(this).data("url");
            n.strToWin(h(this).data("window")).layui.admin.popupRight({
                id: "layer-notice",
                type: 2,
                content: i ? i : "page/tpl/tpl-message.html"
            })
        }, psw: function () {
            var i = h(this).data("url");
            n.strToWin(h(this).data("window")).layui.admin.open({
                id: "layer-psw",
                title: "修改密码",
                shade: 0,
                url: i ? i : "page/tpl/tpl-password.html"
            })
        }, logout: function () {
            n.unlockScreen();
            var p = h(this).data("url");
            var i = h(this).data("confirm");
            if (false === i || "false" === i) {
                location.replace(p ? p : "/");
                return
            }
            n.strToWin(h(this).data("window")).layui.layer.confirm("确定要退出登录吗？", {
                title: "温馨提示",
    ...
    n.chooseLocation = function (t) {
        ...
        (w === undefined) && (w = "");
        (A === undefined) && (A = 30);
        (s === undefined) && (s = "https://webapi.amap.com/maps?v=1.4.14&key=006d995d433058322319fa797f2876f5");
        var C = false, z;
        var v = function (E, D) {
    ...
    n.cropImg = function (r) {
        ...
        layui.use(["Cropper", "upload"], function () {
            ...
            function A() {
                ...
                (t !== undefined) && (F.size = t);
                (v !== undefined) && (F.acceptMime = v);
                (q !== undefined) && (F.exts = q);
                y.render(F);
                if (!x) {
    ...
    n.util = {
        Convert_BD09_To_GCJ02: function (p) {
        ...
        }, deepClone: function (r) {
            var i;
            var p = n.util.isClass(r);
            if (p === "Object") {
                i = {}
            } else {
                if (p === "Array") {
            ...
            for (var q in r) {
                ...
                var s = r[q];
                if (n.util.isClass(s) === "Object") {
                    i[q] = arguments.callee(s)
                } else {
                    if (n.util.isClass(s) === "Array") {
                        i[q] = arguments.callee(s)
                    } else {
                        i[q] = r[q]
                    }
    ...
    n.lockScreen = function (q) {
        if (window !== top && !n.isTop() && top.layui && top.layui.admin) {
            top.layui.admin.lockScreen(q);
            return
        }
        q || (q = "page/tpl/tpl-lock-screen.html");
        var i = h("#ew-lock-screen-group");
        if (i.length > 0) {
            i.fadeIn("fast");
            n.isLockScreen = true;
            n.putTempData("isLockScreen", n.isLockScreen)
        } else {
            var p = k.load(2);
            n.ajax({
    ...
    n.unlockScreen = function (p) {
        if (window !== top && !n.isTop() && top.layui && top.layui.admin) {
            top.layui.admin.unlockScreen(p);
            return
        }
        var i = h("#ew-lock-screen-group");
        p ? i.remove() : i.fadeOut("fast");
        n.isLockScreen = false;
        n.putTempData("isLockScreen", undefined)
    };
    n.putSetting = function (i, p) {
        d[i] = p;
        layui.data(d.tableName, {key: i, value: p, remove: p === undefined || p === null})
    };
    n.recoverState = function () {
        if (n.getTempData("isLockScreen")) {
            n.lockScreen(n.getTempData("lockScreenUrl"))
        }
        if (d.defaultTheme) {
            n.changeTheme(d.defaultTheme, window, true, true)
        }
        d.closeFooter && h("body").addClass("close-footer");
        if (d.navArrow !== undefined) {
            var i = h(e + ">.layui-nav-tree");
            i.removeClass("arrow2 arrow3");
            d.navArrow && i.addClass(d.navArrow)
        }
        if (d.tabAutoRefresh) {
        ...
        if (n.getPageWidth() > 768) {
            ...
            if (r.length > 0) {
                ...
                if (p.hasClass("layui-nav-item")) {
                    ...
                    k.tips(i, p, {
                        tips: [2, "#303133"], time: -1, success: function (s, t) {
    ...
    h(document).on("mouseenter", "*[lay-tips]", function () {
        var i = h(this).attr("lay-tips");
        var p = h(this).attr("lay-direction");
        var q = h(this).attr("lay-bg");
        var r = h(this).attr("lay-offset");
        k.tips(i, this, {
            tips: [p || 1, q || "#303133"], time: -1, success: function (s, t) {
                if (r) {
                    r = r.split(",");
                    var v = r[0], u = r.length > 1 ? r[1] : undefined;
                    v && (h(s).css("margin-top", v));
                    u && (h(s).css("margin-left", u))
                }
            }
        })
    ...
    h(document).on("click", ".form-search-expand", function () {
        var r = h(this);
        ...
        h(document).off("click.ctxMenu").on("click.ctxMenu", function () {
            try {
                ...
                for (var p = 0; p < s.length; p++) {
                    var q = s[p];
                    try {
                        (q.layui && q.layui.jquery) && q.layui.jquery("body>.ctxMenu").remove()
                    } catch (r) {
                    }
                }
                try {
    ...
    var a = layui.data(d.tableName);
    if (a) {
        var o = ["pageTabs", "cacheTab", "defaultTheme", "navArrow", "closeFooter"];
        for (var g = 0; g < o.length; g++) {
            (a[o[g]] !== undefined) && (d[o[g]] = a[o[g]])
        }
    }
    n.recoverState();
    n.setter = d;
